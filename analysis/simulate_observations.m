function [sci, sources] = simulate_observations(star, bandno, outputdir, withPlanet, phaseError, phaseOffset)

if nargin < 4 || isempty(withPlanet)
    withPlanet = false;
end
if nargin < 6
    phaseOffset = 0;
...
% phase_dir0 = 'oneoverf_128_52596_r1_0.400000';
% phase_dir1 = 'oneoverf_128_52596_r1_1.400000';
...
%    stars(1) = star_initialize('acenA', -1, filter, orbitA, ts/86400.);
   stars(1) = star_initialize('G2', 2, filter, orbitA, ts/86400.);
else
...
mkdir(folderout);
sci.psf = fits_read([folderout '/psf.fits']);
sci.psfFwhm = 3.62; % pixels, from VIPER analysis
...
%     nsamp = ts(n)/dt+1;
%     abber = fits_read(sprintf('%s/frame_%05d.fits', phase_dir0, frameStart + n));    
%     abber2 = fits_read(sprintf('%s/frame_%05d.fits', phase_dir1, 8*n));
    abber = phaseError(:,:,phaseOffset + n);   
    optics.primary.aberrations = abber* mirror_figure;
%     optics.primary.aberrations = phase_error(optics.primary.N, 2, 0.1e-9/sqrt(3.7)); % last parameter is rms in m
...
%     fits_write(filename, sci.im, head);
end
laststat = status_line('', laststat);
fprintf(1, 'Simulation complete\n');
    
for i=1:size(sci.im, 3)
    sci.mim(:,:,i) = sci.im(:,:,i) - mean(mean(sci.im(:,:,i))); 
end
